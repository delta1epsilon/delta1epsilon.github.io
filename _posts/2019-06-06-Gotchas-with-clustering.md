...
tags: [clustering, k-means, curse of dimensionality, high-dimensional data, clustering tendency, clustering evaluation, silhouette score, correlated features]
mathjax: true
---
...
When the points are projected onto one axis - half of them got into unit sized bin. When the same projections are stretched into the second dimension - about quarter of them fall into a unit sized bin. And only one eighth of the points fall into unit sized bin in three dimensions. The points continue to spread out, when more dimensions are being added, until they are equidistant from each other and distance is not very meaningful.

So, what are the alternative to traditional clustering algorithms for clustering high-dimensional datasets?

[Subspace clustering](http://mlwiki.org/index.php/Subspace_Clustering) can be used for clustering high-dimensional data. It extends traditional clustering algorithms by finding clusters in different subspaces of the given high-dimensional space, where a subspace is defined using a subset of attributes in the full space.
...
The figure below shows an example of how k-means works when input data doesn't conform to some implicit assumptions that k-means makes - see [Demonstration of k-means assumptions](https://scikit-learn.org/stable/auto_examples/cluster/plot_kmeans_assumptions.html).


![](https://delta1epsilon.github.io/assets/gotchas_w_clustering/k-means_assumptions.png)
...
There is no recognized universal measure for evaluating clustering results. However, measures which consider how well the clusters are separated and how compact the clusters are can be used. [Silhouette coefficient](https://en.wikipedia.org/wiki/Silhouette_(clustering)) is one of such measures.

Silhouette coefficient measures how similar a sample is to other samples in the same cluster and how different it is to samples in other clusters. It's calculated for each sample in a dataset and its value is in between -1 and 1:
...
![](https://delta1epsilon.github.io/assets/gotchas_w_clustering/kmeans_silhouette_analysis.png)
...
* L. Parsons, E. Haque, H Liu. Subspace Clustering for High Dimensional Data: A Review https://www.kdd.org/exploration_files/parsons.pdf

* J. Han, Data Mining: Concepts and Techniques

* From the Euclidean distance over standardized variables to the Mahalanobis distance https://milania.de/showcase/From_the_Euclidean_distance_over_standardized_variables_to_the_Mahalanobis_distance
